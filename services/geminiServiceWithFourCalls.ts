// Gemini Service with Proper Four-Call Orchestration
...
function composeBriefingPrompt(cards: any[], metrics: any, agencyCode: string): string {
  const cardsText = cards.slice(0, 24).map(card => 
...
function composePlaysPrompt(cards: any[], agencyCode: string): string {
  const cardsText = cards.slice(0, 18).map(card =>
...
**Play Name**: [≤5 words]
- Offer: [≤25 words]
- Proof Point: [specific capability, ≤25 words]
- Assets: [key differentiators, ≤25 words]
- Vehicles: [8(a), SEWP, CIO-SP4, etc.]
- Demo: [first meeting show, ≤25 words]
- Success Metric: [measurable outcome, ≤25 words]
...
function composeAnnexPrompt(cards: any[], agencyCode: string): string {
  const cardsForAnnex = cards.slice(0, 30);
  const cardsText = cardsForAnnex.map(card =>
    `[${card.id}] "${card.quote}" | ${card.source_doc}`
  ).join('\n');
...
export async function generateExecutiveBriefingWithFourCalls(
  ...
  try {
    ...
    if (needsDistillation) {
      ...
      const distillationResult = await runDistillationPipeline({
        files: docFiles,
        csvFile,
      ...
      if (distillationResult.success && distillationResult.evidenceCards) {
        evidenceCards = distillationResult.evidenceCards;
        results.distillationUsed = true;
        results.distillationStats = distillationResult.manifest?.stats;
        onProgress?.('DISTILLATION', 'ok', `Distilled to ${evidenceCards.length} evidence cards`);
      } else {
    ...
    console.log(`Cards=${evidenceCards.length} PromptSizes calculating...`);
    ...
    return {
      ...
      runId,
      promptLength: 0, // Sum of all prompts
